...
class StatServer:
    ...
    def __init__(self, statserver_file_name=None, distrib_nb=0, feature_size=0, index=None, ubm=None):
        ...
        if ubm is not None:
            distrib_nb = ubm.weights_.shape[0]
            feature_size = ubm.means_.shape[1]
    ...
    def accumulate_stat(self, ubm, feature_server, seg_indices=None, channel_extension=("", "_b"), num_thread=1):
        ...
        sv_size = ubm.means_.shape[0]*ubm.means_.shape[1]
        if (seg_indices is None) \
                or (self.stat0.shape[0] != self.segset.shape[0]) \
                or (self.stat1.shape[0] != self.segset.shape[0]):
            self.stat0 = numpy.zeros((self.segset.shape[0], ubm.means_.shape[0]), dtype=STAT_TYPE)
        ...
        for count, idx in enumerate(seg_indices):
            ...
            if not ubm.dim() == data.shape[1]:
                ...
                self.stat1[idx, :] = numpy.reshape(numpy.transpose(numpy.dot(data.transpose(), pp)), sv_size).astype(STAT_TYPE)
    ...
    def accumulate_stat_per_speaker(self, ubm, input_feature_filename, idmap, feature_server, channel_extension=("", "_b")):
        ...
        for idx, (spk_id, data) in enumerate(features_per_speaker.items()):
            ...
            # Verify that frame dimension is equal to gmm dimension
            if not ubm.means_.shape[1] == cep.shape[1]:
                raise Exception('dimension of ubm and features differ: {:d} / {:d}'.format(ubm.means_.shape[1], cep.shape[1]))
            else:
                if ubm.invcov.ndim == 2:
    ...
    def adapt_mean_map(self, ubm, r=16, norm=False):
        ...
        mean_supervector = ubm.means_.flatten()

        index_map = numpy.repeat(numpy.arange(ubm.means_.shape[0]), ubm.means_.shape[1])
        ...
        M = alpha[:, index_map] * M + (1 - alpha[:, index_map]) * \
                                      numpy.tile(mean_supervector, (M.shape[0], 1))
    ...
    def adapt_mean_map_multisession(self, ubm, r=16, norm=False):
        ...
        M[numpy.isnan(M)] = 0  # Replace NaN due to divide by zeros
        M = alpha[:, index_map] * M \
            + (1 - alpha[:, index_map]) * numpy.tile(mean_supervector, (M.shape[0], 1))

        if norm:
            if len(ubm.precisions_.shape) == 2:
                # Normalization corresponds to KL divergence
                w = numpy.repeat(ubm.weights_, ubm.means_.shape[1])
                KLD = numpy.sqrt(w * ubm.precisions_.flatten())
    ...
    def ivector_extraction_eigen_decomposition(self,
        ...
        assert ubm.precisions_.flatten().shape[0] == Tnorm.shape[0], \
            'UBM and TV matrix dimension not consistent'
        if delta.shape == (0, ):
            delta = numpy.zeros(ubm.precisions_.flatten().shape)
        assert ubm.precisions_.flatten().shape[0] == delta.shape[0], \
            'Minimum divergence mean and TV matrix dimension not consistent'
    ...
    def factor_analysis(self, rank_f, rank_g=0, rank_h=None, re_estimate_residual=False,
        ...
        if ubm is None:
            ...
            mean = ubm.means_.flatten()
            Sigma_obs = 1. / ubm.precisions_.flatten()
            if F_init is None:
